...
\vspace{15mm}
\begin{center}
{\LARGE{\bf Rete Residuale per la Rimozione di Rumore Poissoniano e Gaussiano da Immagini Mediche}}\\
\vspace{3mm}
\end{center}
\vspace{40mm}
\par
...
Durante la generazione di immagini a raggi X o di immagini tomografiche ci sono molti fattori che contribuiscono alla degradazione dell' immagine finale.
In particolare negli ultimi anni si \`{e} prestata molta attenzione nel cercare di ridurre la dose di radiazioni a cui \`{e} sottoposto il paziente, ma questo genera un aumento del rumore "quantico" dell' immagine dovuto dall' interazione dei fotoni con i recettori del macchinario.\\
Nel primo capitolo vengono descritte dunque quali sono le sorgenti di rumore e di come il rumore possa essere approssimato tramite modelli probabilistici.\\
Successivamente, viene fatta una panoramica dei metodi "classici" per la rimozione del rumore e viene introdotto il concetto di rete neurale e di come il deep learning possa essere utilizzato per approcciare il problema.\\ Nel secondo capitolo viene descritta una particolare rete per il denoising (DN-Resnet), formata da blocchi residui di convoluzioni inseriti per steps durante il training della rete. La rete originale utilizza oltre all' errore quadratico medio utilizza una funzione di errore basata sui filtri di Sobel. Viene poi sperimentata, in aggiunta a queste due funzioni, una terza funzione di loss (perceptual) basata sulle feature map di un modello (Resnet-50) gia trainato su un dataset di immagini molto grandi, per cercare di migliorare la sfocatura indotta da MSE.\\
Nel terzo capitolo vengono riportati i risultati del denoising e della perceptual loss, utilizzando come criterio di confronto SSIM e PSNR. Dai risultati numerici ottenuti si osserva che per valori alti di rumore la perceptual loss aiuti a migliorare la ricostruzione dell' immagine.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%non numera l'ultima pagina sinistra
\clearpage{\pagestyle{empty}\cleardoublepage}
\chapter{Rumore nelle immagini mediche}                %crea il capitolo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%imposta l'intestazione di pagina
\lhead[\fancyplain{}{\bfseries\thepage}]{\fancyplain{}{\bfseries\rightmark}}
\pagenumbering{arabic}                  %mette i numeri arabi
\section{Medical Imaging}                 %crea la sezione
...
\chapter{Rete Residuale per la Rimozione del Rumore : DN-Resnet}
  ...
  \section{DN-Resnet}
    DN-Resnet (Denoising Residual Network) \cite{ren_dn-resnet_2018} consiste in una rete neurale composti da blocchi residui (ResBlocks), inseriti gradualmente nella rete durante il training. Questa strategia di training permette alla rete di convergere pi\`{u} velocemente. Oltre al convenzionale \textbf{errore quadratico medio} (MSE) si utilizza anche un funzione di loss \textbf{edge-aware}. In aggiunta viene sperimentata una terza funzione, la \textbf{perceptual loss} basata su Resnet-50 \cite{he_deep_2015}.\\
    Aggiungendo in cascata 5 Resblocks la rete raggiunge performance buone performance su tutti e tre i tipi di rumore, Gaussian, Poisson e Poisson-Gaussian. Rispetto ad altre reti neurali proposte per il problema del denoising, DN-Resnet ha un tempo di training molto minore e pu\`{o} essere eseguita in tempo reale.
    ...
    \begin{figure}
      \begin{center}
        \includegraphics[scale=0.5]{resblocks.png}
    ...
    \begin{figure}
      \begin{center}
        \includegraphics[scale=0.6]{cascadetraining.png}
  ...
  \begin{figure}
    ...
    \begin{subfigure}{.5\textwidth}
      \centering
      \includegraphics[scale= 0.4]{mias_original_sobel.png}
      \caption{}
      \label{fig:original_sobel}
    \end{subfigure}%
    \begin{subfigure}{.5\textwidth}
      \centering
      \includegraphics[scale=0.4]{sobel.png}
      \caption{}
      \label{fig:sobel}
    \end{subfigure}
    \caption[Edge-Map ottenuta con filtri di Sobel]{La figura (a) mostra una mammografia, mentre la figura (b) mostra il risultato del calcolo della magnitudine del gradiente della luminosita, ottenuto con i filtri di Sobel}
    \label{fig:sobel_}
  \end{figure}
  ...
  \begin{figure}
    \includegraphics[scale = 0.5]{multiscale.png}
...
\chapter{Risultati}
  \section{Datasets}
    Sono stati usati due dataset per valutare il modello, mini-MIAS contenente immagini di mammografie (MMM)\cite{suckling_mammographic_1994} e un dataset di panoramiche dentali (DX) \cite{abdi_panoramic_2017} . MMM \`{e} composto di 322 immagini di risoluzione $1024 \times 1024$ divise in 258 per il training e 64 per validation, mentre da DX sono state usate 116 immagini di risoluzione varia, divise in 93 per il training e 23 per il validation. Entrambi i dataset sono stati scalati ad una risoluzione $512 \times 512$.\\
    Per ridurre la dimensione della rete da ogni immagine sono state estratte delle patch $64 \times 64$. Il denoising avviene quindi sulle singole sottoimmagini e successivamente viene ricostrutia l' immagine originale. Questo metodo genera una discontinuit\`{a} tra i valori dei pixel ai bordi delle sezioni, per ovviare a questo problema le patch vengono estratte dalle immagini usanto 8 pixel di overlap tra una sezione e l' altra. Nella ricostruzione dell' immagine intera, dati gli 8 pixel sovrapposti, vengono presi 4 pixel da una patch e 4 pixel dall' altra per ogni coppia di sottoimmagini con pixel sovrapposti.\\
    ...
    \begin{figure}
      ...
      \begin{subfigure}{.5\textwidth}
        \centering
        \includegraphics[scale= 0.4]{original_mias.png}
        \caption{}
        \label{fig:original_mias}
      \end{subfigure}%
      \begin{subfigure}{.5\textwidth}
        \centering
        \includegraphics[scale=0.4]{poisson_mias.png}
        \caption{}
        \label{fig:poisson_mias}
      \end{subfigure}
      \caption[Esempio di Rumore di Poisson generato con Knuth]{La figura (a) mostra una mammografia presente nel dataset MMM senza rumore, mentre la figura (b) mostra la stessa immagine con un rumore di Poisson generato con la tecnica di Knuth}
      \label{fig:original_noise}
    \end{figure}

  \section{Valutazione del denoising}
    Il modello \`{e} stato implementato in tensorflow usando "adam" come ottimizzatore con parametri di default e per 10 epoche ad ogni step di training.
    ...
    \begin{table}
      \begin{center}
        \begin{tabular}{|c|c|c|c|}
          ...
          \hline
          Gaussian & \begin{tabular}{@{}c@{}}0.05\\0.1\end{tabular}& \begin{tabular}{@{}c@{}} \color{blue} $0.958 \pm 0.008/41.60 \pm 1.434$ \\ $0.928 \pm 0.008/39.06 \pm 1.339$ \end{tabular} & \begin{tabular}{@{}c@{}} $0.957 \pm 0.009/41.34 \pm 1.465$\\ \color{red}$0.932 \pm 0.009/39.15 \pm 1.344$ \\ \end{tabular}\\
          \hline
          Poisson & - & $0.973 \pm 0.009/40.92 \pm 1.510$ & $0.973 \pm 0.009/40.87 \pm 1.492$\\
          \hline
          Poisson - Gaussian & \begin{tabular}{@{}c@{}}0.01\\0.02\\0.05\\0.1\end{tabular} & \begin{tabular}{@{}c@{}} $0.968 \pm 0.009/40.33 \pm 1.523$\\ \color{blue} $0.967 \pm 0.009/40.26 \pm 1.492$ \\  $0.949 \pm 0.010/39.15 \pm 1.451$ \\ $0.913 \pm 0.010/36.97 \pm 1.349$ \end{tabular} & \begin{tabular}{@{}c@{}}\color{red}$0.969 \pm 0.009/40.37 \pm 1.150$ \\ $0.958 \pm 0.008/39.92 \pm 1.151$ \\ $0.949 \pm 0.010/38.86 \pm 1.446$ \\ \color{red} $0.925 \pm 0.012/37.48 \pm 1.343$ \end{tabular}\\
          \hline
        \end{tabular}
        \caption[SSIM/PSNR per diversi rumori e funzioni di loss]{Valutazione su SSIM/PSNR (db) $\pm \sigma$ di DN-Resnet a 13-layer con differenti funzioni di loss, per diversi tipi di rumore. "edge" indica l'utilizzo di mse e edge-loss come funzioni con peso $w_{e} = 0.025$, "edge + perceptual" indica l'utilizzo della compound loss \ref{eq:compound loss} con pesi $w_{e} = 0.025, w_{p} = 0.01$. Sono marcati in blue/rosso valori per cui "edge" e "edge + perceptual" performano meglio rispettivamente, considerando SSIM come criterio}
        \label{tab:ssim/psnr}
      \end{center}
    \end{table}

    La tabella \ref{tab:ssim/psnr} mostra la media di SSIM e PSNR e deviazione standard calcolata sul dataset di validazione di MMM. La rete trainata con Perceptual Loss e edge-loss migliora i valori di SSIM di \textbf{0.004} / \textbf{0.012} per i valori pi\`{u} alti di rumore.

    \begin{figure}
      \centering
      \begin{subfigure}{.4\textwidth}
        \centering
        \includegraphics[scale= 0.3]{original_dental.png}
        \caption{originale}
      \end{subfigure}%
      \begin{subfigure}{.4\textwidth}
        \centering
        \includegraphics[scale=0.3]{noise_dental_poisson.png}
        \caption{poisson ssim $= 0.688$}
      \end{subfigure}
      \begin{subfigure}{.4\textwidth}
        \centering
        \includegraphics[scale=0.3]{recon_dental_poisson.png}
        \caption{ricostruzione con edge + perceptual, ssim $= 0.933$}
      \end{subfigure}
      \caption[Esempio di ricostruzione di una panoramica dentale]{Un esempio di ricostruzione di una panoramica dentale}
      \label{fig:esempio_recon}
    \end{figure}
    ...
    \begin{figure}
      ...
      \begin{subfigure}[b]{1\textwidth}
        \centering
        \includegraphics[width=1\linewidth]{patches_dental3.png}
        \caption{}
      \end{subfigure}
      \begin{subfigure}[b]{1\textwidth}
        \centering
        \includegraphics[width=1\linewidth]{patches_dental2.png}
        \caption{}
      \end{subfigure}
      \caption [Dettagli di ricostruzione]{In figura sono mostrati dei dettagli dell' immagine \ref{fig:esempio_recon}. La rete riesce a ripristinare correttamente la luminosit\`{a} originale e recuperare alcuni dettagli dei nervi e delle radici dentali.}
      \label{fig:dettagli_ricostruzione}
    \end{figure}

    \begin{figure}
      \includegraphics[width = 1\linewidth]{edge_vs_perc.png}
      \caption[Differenze tra le ricostruzioni]{La prima riga mostra le patch originali, la seconda la ricostruzione con edge + perceptual e la terza riga la ricostruzione solo con edge. Edge + perceptual riesce a ridurre in parte le sfocatura dei dettagli.}
      \label{fig:edge_vs_perc}
    \end{figure}
...
Sono stati descritti quali sono i problemi legati all' imaging medico, in particolare quali sono le sorgenti di rumore nella generazione delle immagini.\\
Vengono spiegate quali sono le tecniche classiche per approcciare il problema del denoising e viene data un introduzione di come il "deep learning" pu\`{o} essere usato come medoto per il denoising.\\ Successivamente viene descritta una rete neurale (DN-Resnet) che utilizza dei blocchi residui e un training a step per migliorare la velocit\`{a} di convergenza del modello. In aggiunta alle funzioni di loss gia presenti nella rete, MSE e una funzione di loss basata sui filtri di Sobel, viene presentata una terza funzione che utilizza le feature map di un modello gia trainato (Resnet-50 sul dataset Imagenet), per calcolare la similitudine nello spazio delle feature delle due reti.\\
La rete ottiene buoni risultati anche senza tuning degli iper-parametri, utilizzando dataset molto ridotti (n = 258, n=93) e un numero limitato di epoche di training (n = 10) per step. L' utilizzo della perceptual loss migliora le performance della rete in caso di rumore molto forte (gauss, poisson + gauss, $\sigma = 0.1$), migliorando SSIM di 0.04 - 0.12.
...
\thispagestyle{empty}
Ringrazio la Prof. Elena Loli Piccolomini per avermi proposto ed indirizzato sull' argomento e il dott. Davide Evangelista per il supporto durante la fase di implementazione. Ringrazio infine tutte le persone che mi hanno supportato e sopportato durante questo percorso di laurea.
\end{document}
