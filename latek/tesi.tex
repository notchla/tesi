...
Vengono descritte dunque quali sono le sorgenti di rumore e di come il rumore possa essere approssimato tramite modelli probabilistici.\\
Successivamente, viene fatta una panoramica dei metodi "classici" per la rimozione del rumore e viene introdotto il concetto di rete neurale e di come il deep learning possa essere utilizzato per approcciare il problema.\\ Viene poi descritta una particolare rete per il denoising (DN-Resnet), formata da blocchi residui di convoluzioni inseriti per step durante il training della rete. La rete originale utilizza oltre all' errore quadratico medio una funzione di errore basata sui filtri di Sobel. In aggiunta a queste due funzioni viene inserita nella rete una terza funzione di loss (perceptual) basata sulle feature map di un modello (Resnet-50) gia trainato su un dataset di immagini molto grandi, per cercare di migliorare la sfocatura indotta da MSE.\\
Utilizzando come criterio di confronto SSIM e PSNR dai risultati ottenuti si osserva che per valori alti di rumore la perceptual loss aiuti a migliorare la ricostruzione dell' immagine.
...
\chapter{Risultati}
  ...
  \section{Valutazione del denoising}
    ...
    \begin{figure}
      ...
      \end{subfigure}
      \caption[Esempio di ricostruzione di una panoramica dentale]{}
      \label{fig:esempio_recon}
    \end{figure}

    \begin{figure}
      \begin{center}
        \includegraphics[scale=0.4]{patches_dental.png}
...
\thispagestyle{empty}
Ringrazio la Professoressa Elena Loli Piccolomini per avermi proposto ed indirizzato sull' argomento e il dottorando Davide Evangelista per il supporto durante la fase di implementazione. Ringrazio infine tutte le persone che mi hanno supportato e sopportato durante questo percorso di laurea.
\end{document}
